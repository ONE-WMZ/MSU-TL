...
# ! huber loss
def huber_loss(pred, target, mask, beta=1):
...
# ! mask data(random)
def mask_(masked_data, mask_ratio:float, mask_value=0.0):
...
# ! mask(random channel)
def mask_channel_random_(masked_data, mask_ratio: float, mask_value=0.0):
    if isinstance(masked_data, torch.Tensor):
        masked_data = masked_data.clone()
        B, C, T = masked_data.shape
        channel_mask = torch.rand(B, C) < mask_ratio  # [B, C]
        mask = channel_mask.unsqueeze(-1).expand(-1, -1, T)
        masked_data[mask] = mask_value
        return masked_data, mask
    elif isinstance(masked_data, np.ndarray):
        masked_data = masked_data.copy()
        B, C, T = masked_data.shape
        channel_mask = np.random.rand(B, C) < mask_ratio
        mask = np.repeat(channel_mask[:, :, np.newaxis], T, axis=2)
        masked_data[mask] = mask_value
        return masked_data, mask
    else:
        raise TypeError("Input must be a NumPy array or PyTorch tensor.")
...
# ! Multi-scale module(Attention-weighted)
# class multi_(nn.Module):
#     def __init__(self, ch_in, ch_out, reduction=4):
#         super().__init__()
#         self.conv_size_5 = nn.Conv1d(ch_in, ch_out, kernel_size=5, padding=2)  
#         self.conv_size_10 = nn.Conv1d(ch_in, ch_out, kernel_size=11, padding=5)        
#         self.conv_size_20 = nn.Conv1d(ch_in, ch_out, kernel_size=21, padding=10)               
...
#             nn.Linear(ch_out * 4, ch_out * 4 // reduction),
#             nn.ReLU(),
#             nn.Linear(ch_out * 4 // reduction, 4),
#             nn.Softmax(dim=1)
#         )
#         self.relu = nn.ReLU()
...
#         x_20 = self.conv_size_20(x)
#         x_hollow = self.conv_hollow(x)
#         concat = torch.cat([x_5, x_10, x_20, x_hollow], dim=1)
#         weights = self.fc(self.global_pool(concat).squeeze(-1)).unsqueeze(-1)
#         out = (x_5 * weights[:, 0:1] + 
#                x_10 * weights[:, 1:2] + 
#                x_20 * weights[:, 2:3] + 
#                x_hollow * weights[:, 3:4])
