...
from model_.unit_ import set_seed, device, huber_loss, mask_, mask_channel_random_
...
best_pretrain_path = f"best_pretrain.pth"
pre_dataset = getsome_(root_dir=data_path)
...
for epoch in range(pre_epoch):
    ...
    for input_x, _ in progress_bar:
        opt_pre.zero_grad()  
        masked_data, mask = mask_(input_x, mask_ratio=mask_radio)
...
for i in users_list:
    ...
    test_dataset = getsome_(root_dir=data_path, only_subjects=[i])
    ...
    # ! finetune
    # optimizer_clf = optim.Adam(list(pre_model.parameters()) + list(class_model.parameters()), lr=0.001)
...
metrics_to_calc = ["Accuracy", "Sensitivity", "Specificity", "Precision", "F1 Score", "AUC"]
